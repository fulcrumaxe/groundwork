...
def skeleton_html(rows=DEFAULT_ROWS) -> str:
    ...
    try:
        ...
        lesson_rows = "".join(
            ...
            "<span class='sk-line sk-row-title'></span>"
        ...
        return (
            ...
            "</div>"
            "<div class='bar sk-progress'><i></i></div>"
            + lesson_rows +
            "</div></div>"
        )
...
def skeletons_css() -> str:
    ...
    try:
        return (
            ...
            ".sk-sub{max-width:85%}"
            ".sk-titlebar{background:#e6e6e6}"
            ".bar.sk-progress i{display:block;width:40%;background:#cfcfcf;"
            "min-height:.5rem;border-radius:4px}"
            ".sk-row{border:1px solid #ddd;border-radius:10px;"
            "padding:.75rem 1rem;margin:.75rem 0}"
            ".sk-row-title{max-width:45%}"
